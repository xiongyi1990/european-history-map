...
out=root/'blender'/'lesson-02';out.mkdir(parents=True,exist_ok=True)
config=json.loads((root/'scripts'/'lesson02-scenes.json').read_text(encoding='utf-8'))
scenes=[(s['id'],s['bounds'],s['zoom'],s['cols'],s['rows']) for s in config['scenes']]
...
for name,bounds,z,cols,rows in scenes:
    ...
    def tile(job):
        ...
        candidates=[root/'public'/'roman-surface'/rel,root/'public'/'greek-surface'/rel,root/'blender'/'lesson-01'/'terrain-cache'/rel,cache/rel]
