...
export function HistoryDetailCard({place,detail,places,year,onYear,onSelect,onBack,region,onRegion}:{region?:{id:string;name:string};onRegion?:(id:string)=>void;place:GazetteerPlace;detail:HistoricalDetail;places:GazetteerPlace[];year:number;onYear:(y:number)=>void;onSelect:(id:string)=>void;onBack:()=>void}){
 return <article className="e-detail h-detail"><button className="e-back" onClick={onBack}>← 返回地点索引</button>
  ...
  {region&&onRegion&&<div className="h-related"><button onClick={()=>onRegion(region.id)}>查看所在地域：{region.name}<span>地域组成、居民与邻近城市 →</span></button></div>}
